    ...
    <Box sx={{ maxWidth: 420, mx: 'auto', mt: 4 }}>
      <Card sx={{ p: 2, borderRadius: 3, boxShadow: 3 }}>
        <CardContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
            ...
            <Typography color="text.secondary">{t('Brewery')}: {user?.brewery_name || t('Not assigned') || 'Sin asignar'}</Typography>
            <Typography color="text.secondary">{t('Email')}: {user?.sub}</Typography>
      ...
      <Dialog open={showPwd} onClose={() => setShowPwd(false)} maxWidth="xs" fullWidth>
        ...
        <form onSubmit={handleChangePassword}>
          ...
          <DialogActions>
            <Button onClick={() => setShowPwd(false)}>{t('Close')}</Button>
