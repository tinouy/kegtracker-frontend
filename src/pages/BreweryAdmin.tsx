...
const BreweryAdmin: React.FC = () => {
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: 'auto', p: 2, position: 'relative' }}>
      <Typography variant="h4" gutterBottom>{selectedBrewery ? selectedBrewery.name : t('brewery_admin') || 'Administrar tu cervecería'}</Typography>
      {selectedBrewery && (
        <>
          {/* Usuarios */}
          <Box mb={6}>
            <Box display="flex" alignItems="center" justifyContent="space-between" mb={1}>
              <Typography variant="h6" gutterBottom>{t('users') || 'Usuarios'}</Typography>
              <Button variant="contained" color="primary" startIcon={<AddIcon />} onClick={() => setShowInvite(true)} />
            </Box>
            <DataGrid
              rows={users}
              columns={[
                ...
                { field: 'role', headerName: t('role') || 'Rol', width: 140 },
                {
                  ...
                  renderCell: (params) => (
                    <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', minHeight: 56 }}>
                      <Fab size="small" color="primary" onClick={() => setUserForm(params.row)} title={t('edit') || 'Editar'}><EditIcon /></Fab>
                      <Fab size="small" color="error" onClick={() => handleDeleteUser(params.row.id)} title={t('delete') || 'Eliminar'}><DeleteIcon /></Fab>
                    </Box>
                  ),
                },
              ]}
          ...
          <Box mb={6}>
            <Box display="flex" alignItems="center" justifyContent="space-between" mb={1}>
              <Typography variant="h6" gutterBottom>{t('kegs') || 'Barriles'}</Typography>
            ...
            <DataGrid
              ...
              columns={[
                ...
                { field: 'name', headerName: t('name') || 'Nombre', flex: 1, minWidth: 180 },
                { field: 'type', headerName: t('type') || 'Tipo', width: 120 },
                { field: 'state', headerName: t('state') || 'Estado', width: 120, renderCell: (params) => params.value === 'in_use' ? 'En Uso' : params.value },
                {
                  field: 'actions',
                  headerName: t('actions') || 'Acciones',
                  width: 120,
                  ...
                  renderCell: (params) => (
                    <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', minHeight: 56 }}>
                      <Fab size="small" color="primary" onClick={() => { setEditKeg(params.row); setShowForm(true); }} title={t('edit') || 'Editar'}><EditIcon /></Fab>
                      <Fab size="small" color="error" onClick={() => handleDeleteKeg(params.row.id)} title={t('delete') || 'Eliminar'}><DeleteIcon /></Fab>
                    </Box>
                  ),
                },
              ]}
          ...
          <Dialog open={showInvite} onClose={() => setShowInvite(false)} maxWidth="xs" fullWidth>
            <DialogTitle>{t('invite_user') || 'Invitar usuario por email'}</DialogTitle>
            <form onSubmit={handleInvite}>
              <DialogContent>
                <TextField
                  type="email"
                  label={t('invite_user') || 'Invitar usuario por email'}
                  value={email}
              ...
              <DialogActions>
                <Button onClick={() => setShowInvite(false)}>{t('close') || 'Cerrar'}</Button>
                <Button type="submit" variant="contained" color="primary" disabled={loading || !email}>{loading ? t('loading') : t('send_invite') || 'Enviar Invitación'}</Button>
              </DialogActions>
            </form>
          </Dialog>
          {/* Modales y feedback */}
          <UserForm 
            ...
            onSuccess={() => { 
              setUserForm(null); 
              setRefresh(r => r + 1); 
              setSuccessMsg(t('saved_ok') || 'Guardado'); 
            }} 
