...
// Estados movidos dentro del componente para usar t()
...
const KegDetail: React.FC = () => {
  ...
  // Definición de estados traducida
  const kegStates = [
  ...
  // Funciones para traducir tipos y conectores
  ...
  const getConnectorLabel = (connector: string) => {
    switch (connector) {
      case 'S': return t('S connector');
      case 'A': return t('A connector');
      case 'G': return t('G connector');
      case 'ball_lock': return t('Ball Lock');
      case 'pin_lock': return t('Pin Lock');
  ...
  if (loading) return <Box display="flex" justifyContent="center" alignItems="center" minHeight="300px"><CircularProgress /></Box>;
      if (error || !keg) return <Box color="red" textAlign="center" py={4}>{error || t('Not found')}</Box>;
